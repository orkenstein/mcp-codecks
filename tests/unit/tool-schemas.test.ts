...
describe("tool schemas", () => {
  it("validates bulk update cards inputs", () => {
    ...
    expect(() => UpdateCardSchema.parse({ card_id: "a", parent_card_id: "parent-1" })).not.toThrow();
    expect(() => UpdateCardSchema.parse({ card_id: "a", parent_card_id: null })).not.toThrow();
    expect(() => UpdateCardSchema.parse({ card_id: "a", child_cards: ["child-1"] })).not.toThrow();
    expect(() => UpdateCardSchema.parse({ card_id: "a", child_cards: [] })).not.toThrow();
